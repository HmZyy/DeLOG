...
- [~] **TDV-04** — PosMapping NED / Custom-with-units; trajectory build off-thread — NED + custom unit scale + GPS mappings resolve to render space (`position_at`, unit-tested); `build_trajectory` resamples the path. The app **caches** trajectories and rebuilds only on epoch/config change (no per-frame work); moving that rebuild **off-thread** (§19.6) is the remaining follow-up
