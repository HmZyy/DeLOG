...
- [~] **TDV-03** — `VehicleConfig` + dialog with per-source mapping presets (§12.1) — data model + the **config dialog** done (`vehicle_dialog.rs`): source picker, NED/GPS + Static/Euler/Quat field-mapping ComboBoxes, unit/model/color/scale, an auto-detect button matching common Lat/Lng/Alt + Roll/Pitch/Yaw names, and a vehicle list (show toggle / remove). Reached from the toolbar "Vehicles" button. Pending in-app verification
