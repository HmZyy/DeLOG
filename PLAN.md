...
- [~] **TDV-03** — `VehicleConfig` + dialog with per-source mapping presets (§12.1) — data model + the **config dialog** done (`vehicle_dialog.rs`): source picker; pick a position topic then its N/E/D (or GPS lat/lon/alt) columns, and an orientation topic then Euler/Quat columns; model/color/scale; auto-detect by common names; vehicle list with show toggle / **edit** / remove. Reached from the toolbar "Vehicles" button. Pending in-app verification
