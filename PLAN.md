...
| MAVLink       | **rust-mavlink** (`all` dialect)                      | Generated message types for AP+PX4-common set; we wrap our own I/O for counters (§7.2).                                                                                                            | Hand parser (only the framing layer is hand-written, for counters).                                                                                                                                         |
...
- [x] **GPU-22** — `mesh` pipeline (N·L+ambient) + GLB upload path — `MeshPipeline` (vertex+index buffers, Lambert N·L + ambient, depth+MSAA, double-sided), `MeshUniform { view_proj, model, normal_mat, light_dir, color, ambient }`, `MeshGpu::upload`. `load_glb` decodes the first triangle primitive via the `gltf` crate (smooth normals when absent, flat indices when absent); `MeshCpu::cone` is the procedural fallback (§12.4). Golden tests prove a shaded gradient renders and a hand-built GLB decodes. App wiring (model registry, embedded GLBs, per-vehicle draw) lands with TDV-03/08
- [x] **GPU-23** — `traj3d` trajectory line pipeline — `Traj3dPipeline`: vertex-pulled line-list from a `vec4` points storage buffer (`(N-1)*2` verts, depth-tested, MSAA), `Traj3dUniform { view_proj, color }`; non-finite endpoints collapse the segment so NaN is a gap (§9.4). Golden tests prove a polyline renders and a NaN endpoint leaves no pixels. Also serves the vertical world-axis line GPU-21 couldn't draw. App consumers (demo path, vehicle trajectories) arrive with TDV-10/11
- [x] **GPU-24** — 3D frame-time metric — `scene_ui` wraps the scene build+encode in a `metrics.scope("3d_frame")` RAII timer (§16 metric name). `Session::metrics()` exposes the registry; `PlotServices` carries it. The metric accumulates now; the perf dock that displays it lands with PRF-02
...
- [x] **PLT-11** — Context menu: remove/clear/mode/color/width/split/reset/legend/info (§10.6) — reworked with Lucide icons: Clear all traces / Remove trace (submenu, per-trace colour) / Edit trace (colour·mode·width submenu) · Split horizontally/vertically · Show legend + Show tooltip checkboxes (per-pane state, persisted) + Hover mode · Plot Info (opens a per-pane window with geometry/timings + per-trace cache/GPU detail, replacing the Debug submenu) · Close. Reset view dropped from the menu (double-click still resets). Tooltip visibility now toggled via `PlotServices::show_tooltip`
...
- [x] **TDV-06** — OriMapping Static/Euler(deg|rad)/Quaternion; prev-sample pose at playhead — `orientation_at` + `geo::euler_to_quat`/`body_to_render_rot` (unit-tested); pose read prev-sample at the playhead in the render path. Dialog configuration verified in-app with Euler ATT roll/pitch/yaw driving a fixed-wing loop maneuver
...
- [x] **TDV-09** — Per-vehicle color/path color/scale/show; multiple vehicles — the scene render path draws N vehicles (mesh at pose + trajectory) honoring color/path-color/scale/show; the dialog creates multiple vehicles; the scene pane shows a tracked-vehicle dropdown when ≥2 vehicles are configured, preserves the selected row, falls back to a visible pose when needed, and uses per-row GPU cache keys so same-source vehicles do not collide
...
- [~] **BRW-06** — Context: plot-in-new-pane / copy key / stats popup / remove source — right-click a source header → "Remove source" wired through `Session::remove_source` → `IngestSender::remove_source` (existing ingest-thread tombstone + epoch republish + cache GC). Pending: plot-in-new-pane, copy key, stats popup
...
- [x] **ANA-02** — Visible-window stats: existing Field Stats popup now has **Visible window** (default) and **Global** tabs. `delog_core::analysis::visible_field_stats` computes exact inclusive-window min/max/mean/σ/count/missing/rate in plotted units, folding sealed `ColStats` for fully covered chunks and scanning only Arrow boundary slices (`// upholds ZC-2`); global stats now apply the same schema multiplier semantics. `FieldStatsController` runs one background job with one newest pending request, 10 Hz launch cap, stale `(field, epoch, window)` rejection, and an 8-result LRU; the popup auto-refreshes during pan/zoom/live updates, shows provisional pyramid min/max, dims stale values under **Updating...**, and renders empty values as `-`. Deterministic + proptest naive-equivalence coverage and controller tests pass. Criterion quick results: ~8.8 µs boundary window / ~4.8 µs across 196 small chunks / ~151 ns fully covered 2M rows. GUI layout approved via temporary mockup; manual in-app run remains pending.
...
- [~] **SCR-11** — User-created Python file parsers: persistent `parsers/*.py` library, Tools ▸ Parsers management/editor UI, explicit open-with-parser action, and unchanged `Parse(numpy.float32[]) -> [(name, values, tooltip)]` compatibility routed through the embedded scripting worker and canonical ingest pipeline. Automated verification (2026-06-20): format, Clippy, no-default-features app check, and `git diff --check` clean; workspace tests 467 passed/0 failed/1 ignored. Criterion (10 samples): read 1.2267–1.2854 ms / 3.0390–3.1843 GiB/s; Python call 185.95–187.05 µs / 5.6935–5.7270 Gelem/s; conversion 714.06–732.64 µs / 1.4536–1.4914 Gelem/s; Python+conversion 692.12–696.98 µs / 1.5280–1.5387 Gelem/s. Launch smoke stayed alive for 15 s without a startup error (timeout exit 124); manual GUI verification remains pending for add, syntax rejection, save/rename/edit/delete confirmation, picker, legacy parse/load/tooltip, traceback, spinner, cancellation, and built-in-parser exclusion, so Definition of Done is not yet met. ZC exceptions are accounted by `python_parser_input_bytes`, `python_parser_output_bytes`, `python_parser_read`, `python_parser_python`, `python_parser_convert`, `python_parser_total`, `python_parser_cancelled`, and `python_parser_failures`. Design: `docs/superpowers/specs/2026-06-20-custom-python-parsers-design.md`; implementation plan: `docs/superpowers/plans/2026-06-20-custom-python-parsers.md`. Output-contract refinement (2026-06-20, `custom_parsers` branch): the third tuple item (tooltip) is now ignored (these parsers are reused from another tool); a trailing `[unit]` in the field name is parsed into the native `FieldSchema.unit` (numeric brackets like `distance[0]` stay as indices), with the original unit-bearing name retained only when stripping would collide within a topic (e.g. `imu.gyr.front [rad/s]` vs `[deg/s]`); and 2-D field values are split along the first axis into per-lane fields suffixed `[0..K-1]` (last axis is the sample axis) instead of being rejected. Re-verified this session: `cargo fmt`, `clippy --workspace -D warnings`, no-default-features app build, and `cargo test --workspace` clean (3 new `custom_parser` unit tests added); criterion benches not re-run.
