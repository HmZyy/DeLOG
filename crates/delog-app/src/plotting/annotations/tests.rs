...
fn geometry_inside_the_view_is_visible() {
    let tf = unit_transform();
    assert!(draw::is_visible(&box_geom(), &tf));
}
...
fn partially_visible_geometry_is_not_skipped() {
    ...
    assert!(draw::is_visible(&straddling, &tf));
}

#[test]
fn hline_outside_the_y_range_is_skipped() {
    let tf = unit_transform();
    assert!(draw::is_visible(&Geometry::HLine { y: 50.0 }, &tf));
    assert!(!draw::is_visible(&Geometry::HLine { y: 900.0 }, &tf));
}
