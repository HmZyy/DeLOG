...
mod tests {
        ...
        let next = presentations
            ...
            .find(|presentation| {
                presentation.command
                    == AppCommand::SetCursorSampling(delog_core::field_view::SampleMode::Next)
            })
            .unwrap();
    ...
    fn menu_and_toolbar_toggle_presentations_share_selected_and_disabled_state() {
        ...
        assert_eq!(
            presentation(CommandId::ToggleDataBrowser).selected,
            Some(true)
        );
        assert_eq!(
            presentation(CommandId::OpenDiagnostics).selected,
            Some(true)
        );
        assert_eq!(
            presentation(CommandId::AddMeasuringMarker).selected,
            Some(true)
        );
        assert_eq!(
