#![allow(dead_code)]

pub const APP: &str = include_str!("../src/app.rs");
pub const BROWSER: &str = include_str!("../src/browser.rs");
pub const DATAFLOW_CANVAS: &str = include_str!("../src/dataflow/canvas.rs");
pub const DATAFLOW_WINDOW: &str = include_str!("../src/dataflow/window.rs");
pub const DATA_EXPORT: &str = include_str!("../src/data_export.rs");
pub const DIAGNOSTICS: &str = include_str!("../src/diagnostics.rs");
pub const DOCKS: &str = include_str!("../src/docks.rs");
pub const GENERATE_MARKERS: &str = include_str!("../src/generate_markers.rs");
pub const HOVER: &str = include_str!("../src/hover.rs");
pub const LEGEND: &str = include_str!("../src/legend.rs");
pub const LIVE: &str = include_str!("../src/live.rs");
pub const LOGGING: &str = include_str!("../src/logging.rs");
pub const MARKERS: &str = include_str!("../src/markers.rs");
pub const MESSAGE_POPUP: &str = include_str!("../src/message_popup.rs");
pub const PARQUET_IMPORT: &str = include_str!("../src/parquet_import.rs");
pub const PARSERS: &str = include_str!("../src/parsers.rs");
pub const PERFORMANCE: &str = include_str!("../src/performance.rs");
pub const SCRIPTS: &str = include_str!("../src/scripts.rs");
pub const SETTINGS: &str = include_str!("../src/settings.rs");
pub const SYNC_WINDOW: &str = include_str!("../src/sync_window.rs");
pub const VEHICLE_DIALOG: &str = include_str!("../src/vehicle_dialog.rs");
pub const WORKSPACE: &str = include_str!("../src/workspace.rs");
pub const APP_MANIFEST: &str = include_str!("../Cargo.toml");
pub const CORE_INGEST: &str = include_str!("../../delog-core/src/ingest.rs");
pub const DATA_FLOW_DOCS: &str = include_str!("../../../docs/data_flow.md");
