//! Canonical microsecond time model. Timestamps and offsets are integer µs;
//! floating point belongs only in render caches.
...
/// Effective source time = raw timestamp + source offset.
...
/// Apply each source offset, then union the effective ranges.
