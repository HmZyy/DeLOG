...
fn node_sublayer_parent(ctx: &egui::Context, scene_layer: egui::LayerId) -> egui::LayerId {
    ctx.memory(|memory| {
        memory
            .areas()
            .parent_layer(scene_layer)
            .unwrap_or(scene_layer)
    })
}
...
impl Node {
    ...
    fn show_impl<'a, R>(
        ...
        // Custom framed node container that follows the scene's transform.
        ...
        let sublayer_parent = node_sublayer_parent(ui.ctx(), scene_layer);
        let node_id = self.id;
        ...
        ui.ctx().set_sublayer(sublayer_parent, socket_layer);
        ...
        ui.ctx().set_sublayer(sublayer_parent, frame_layer);
        if let Some(transform) = ui.ctx().layer_transform_to_global(scene_layer) {
...
mod tests {
    ...
    fn layer(name: &'static str) -> egui::LayerId {
        egui::LayerId::new(egui::Order::Middle, egui::Id::new(name))
    }

    #[test]
    fn node_sublayer_parent_flattens_a_nested_scene() {
        let ctx = egui::Context::default();
        let window_layer = layer("window");
        let scene_layer = layer("scene");
        ctx.set_sublayer(window_layer, scene_layer);

        assert_eq!(node_sublayer_parent(&ctx, scene_layer), window_layer);
    }

    #[test]
    fn node_sublayer_parent_keeps_an_unparented_scene() {
        let ctx = egui::Context::default();
        let scene_layer = layer("scene");

        assert_eq!(node_sublayer_parent(&ctx, scene_layer), scene_layer);
    }
}
